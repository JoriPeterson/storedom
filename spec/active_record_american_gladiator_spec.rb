...
describe "ActiveRecord American Gladiator" do
  ...
  context "Powerball" do
    it "returns all items containing Powerball" do
      ...
      # Changeable Start
			# items = Item.where.not(name: "Trap Door")
			items = Item.where("name LIKE :query", { query: "%Powerball%"})
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      # Changeable Start
			most_popular_items = Item.select("items.*, count(order_items.item_id) AS items_count").joins(:order_items).group("items.id").order("items_count desc").limit(2)
      # Changeable Stop
